...
const userModules = [
  { id: "billingCounter", title: "Billing Counter", icon: "💳", userRole: "Cashier / Billing Staff", purpose: "Fast POS billing for counter staff with full product visibility and kitchen availability status.", permissions: ["Create bill", "Hold bill", "Generate KOT"] },
  { id: "selfKiosk", title: "Self Desk Kiosk", icon: "🖥️", userRole: "Customer Self-Service", purpose: "Customer-facing kiosk ordering screen that hides unavailable products and closed-kitchen items.", permissions: ["Order", "Pay", "Print token"] },
  { id: "kotManagement", title: "KOT Management", icon: "🧾", userRole: "Kitchen Dispatcher", purpose: "Manage kitchen tickets, section-wise queues, preparation status and delivery handover.", permissions: ["View KOT", "Mark ready", "Mark delivered"] },
  { id: "productionManagement", title: "Production + Requisition", icon: "🏭", userRole: "Production Supervisor", purpose: "Convert KOT and batch demand into production orders and raw material requisitions.", permissions: ["Create production", "Request materials", "Close production"] },
  { id: "productionPlanning", title: "Production Planning", icon: "📈", userRole: "Planning Manager", purpose: "Plan item-wise and hour-wise production using average sales, buffer and recipe batches.", permissions: ["Create plan", "Edit forecast", "Approve plan"] },
  { id: "storeManagement", title: "Store Management", icon: "🏬", userRole: "Store Keeper", purpose: "Manage stock issue, shelf life, reorder levels and requisition fulfilment.", permissions: ["Issue stock", "Track expiry", "Raise shortage"] },
  { id: "purchase", title: "Purchase", icon: "🛒", userRole: "Purchase Manager", purpose: "Handle purchase requisition, vendor quotes, PO and GRN comparison.", permissions: ["Create PR", "Compare quote", "Verify GRN"] },
];
...
function getPurchaseRequisition(storeAlerts, requisitions) {
  ...
  storeAlerts.filter((item) => item.reorderStatus === "Reorder").forEach((item) => {
    items.set(`${item.item}-${item.unit}`, { item: item.item, unit: item.unit, qty: item.reorderLevel - item.current, reason: "Reorder level" });
  });
...
function Button({ children, active = false, disabled = false, className = "", ...props }) {
  return (
    <button type="button" disabled={disabled} className={`inline-flex items-center justify-center rounded-full px-5 py-2 text-sm font-semibold transition focus:outline-none focus:ring-4 focus:ring-[#abffae]/60 disabled:cursor-not-allowed disabled:opacity-50 ${active ? "bg-[#00262b] text-white hover:bg-[#0b363b]" : "border border-[#abffae] bg-white text-[#0b363b] hover:bg-[#eafde8]"} ${className}`} {...props}>
      {children}
    </button>
  );
}
...
function RoleModuleContent(props) {
  switch (props.activeModule) {
    ...
    case "productionManagement": return <ProductionManagementModule batchPlan={props.batchPlan} requisitions={props.requisitions} />;
...
export default function FoodCourtCateringManagementApp() {
  ...
  const kotDemand = useMemo(() => getKotDemand(), []);
